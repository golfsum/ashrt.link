...
/**
 * Emergency denylist for destination domains that have been observed abusing
 * this service or are currently identified by security vendors as risky.
 *
 * This list is intentionally small. The normal operator-managed blocklist still
 * lives in lib/abuse.js / Redis. These entries are a last-resort safety net so
 * an already-created malicious short link stops resolving even if the Redis
 * blocklist is missing, stale, or its index has drifted.
 *
 * Keep exact registrable domains here, never broad TLDs or hosting providers.
 */
...
export function isBlockedHost(host, blockedList = []) {
  return [...BUILTIN_BLOCKED_DOMAINS, ...blockedList].some((b) => hostMatches(host, b))
}
...
export { MAX_URL_LENGTH, BLOCKED_PORTS, BUILTIN_BLOCKED_DOMAINS }
