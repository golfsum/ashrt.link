 ...
 */
export function sanitizeRules(input, { max = MAX_RULES } = {}) {
  const errors = []
  if (input === null || input === undefined) return { rules: [], errors }
  if (!Array.isArray(input)) return { rules: [], errors: ['Rules must be a list.'] }
  // The ceiling is per plan; MAX_RULES is the highest any plan allows and the
  // hard cap on what a record may hold.
  const cap = Math.min(max, MAX_RULES)
  if (input.length > cap) errors.push(`Your plan allows at most ${cap} rules on a link.`)

  const rules = []
  for (const [i, raw] of input.slice(0, cap).entries()) {
